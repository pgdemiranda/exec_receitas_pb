...
BQ_TABLE = os.getenv("BQ_TABLE_UN_GESTORA")
...
@dag(
    dag_id="unidade_gestora",
    default_args=default_args,
    schedule_interval="@yearly",
    catchup=True,
)
def unidade_gestora_dag():
    @task()
    def download_task(**kwargs):
        logical_date = kwargs["logical_date"]
        download_csv_from_url(logical_date, conjunto="unidade_gestora", local_csv_path="/tmp/unidade_gestora_dadospb.csv")
        return "/tmp/unidade_gestora_dadospb.csv"
    ...
    def carregar_para_bq_task(**kwargs):
        ...
        carregar_para_bq = GCSToBigQueryOperator(
            ...
            source_objects=[f"unidade_gestora/dadospb_{logical_date.year}.csv"],
